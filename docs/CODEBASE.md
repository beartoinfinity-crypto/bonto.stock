...
- **Live re-marking (leaderboard/positions)**: on page load the browser fetches live quotes for every open-position symbol and re-marks equity/P-L at current prices (a "live" badge shows when live quotes are in; the last day's snapshot is the offline fallback). The settled record is unchanged — fills and history stay at their day's prices.
- **Daily performance history**: each simulated day appends one `ledger.history` entry (`LedgerHistoryEntry`: date + per-persona mark-to-market equity + the day's prices) via `appendHistory` (union by date — a re-run replaces that day). The page renders a **Performance History** table (per-persona daily equity + P/L) and each persona tab shows a daily P/L summary (total/best/worst day). `ledgerMerge` unions history by date (fuller entry wins); **Reset today** drops today's entry. The server-side `simulate-ledger` fn records the same entry.
