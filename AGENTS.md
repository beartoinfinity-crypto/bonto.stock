...
2. Run `npm run build` before committing — `dist/` is committed to git (Render/Vercel serve it via `index.js`)
...
- **Secrets never go in source or `.env`-built bundles** — GitHub push protection blocks them. Keys are served at runtime via `/api/api-keys` (ADANOS_API_KEY) and `/api/edge-config` (EDGE_FN_URL/EDGE_FN_KEY), set on Render. The committed `dist/` bundle is public. `CRON_SECRET` must ALSO be set as a hosting env var on Render/Vercel — the `/api/ledger/*` proxy forwards it to the edge fn (without it those endpoints 502)
